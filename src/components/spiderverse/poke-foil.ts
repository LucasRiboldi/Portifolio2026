 ...
 * A carta se chama:   {ID}_hires.avif (era .png antes da otimização)
 *
 ...
 *                       49_hires.avif   → 049_foil_holo_reverse_2x.webp
 *                       138_hires.avif  → 138_foil_holo_sunpillar_2x.webp
 *   2. prefixo TG/SV  → prefixo mantido, tudo em minúsculo
 *                       TG17_hires.avif → tg17_foil_etched_sunpillar_2x.webp
 *                       SV023_hires.avif→ sv023_foil_etched_sunpillar_2x.webp
 *   3. prefixo SWSH   → o prefixo CAI, sobra só o número
 *                       SWSH181_hires.avif → 181_foil_etched_sunpillar_2x.webp
 *
...
/**
 * Extrai o ID de um caminho tipo `/poke-holo/TG17_hires.avif`.
 *
 * A extensão é casada como grupo aberto e não como `.png` fixo: as faces
 * migraram para AVIF, e um regex preso ao formato antigo devolveria
 * `"138_hires.avif"` em vez de `"138"` — sem erro nenhum, só o foil sumindo de
 * todas as cartas.
 */
export function idDaCarta(img: string): string {
  return img.split("/").pop()?.replace(/_hires\.[a-z0-9]+$/i, "") ?? ""
}
